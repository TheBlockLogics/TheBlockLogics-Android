...
private class JavaLanguageSpec() :
  TsLanguageSpec(
    ...
    readFromAsset("editor/treesitter/java/highlights.scm"),
  )

fun TsThemeBuilder.buildJavaTheme() {
  makeStyle(COMMENT, 0, false, true, false) applyTo "comment"
  makeStyle(KEYWORD, 0, true, false, false) applyTo
    arrayOf("variable.builtin", "function.builtin", "type.builtin", "keyword")
  makeStyle(LITERAL) applyTo arrayOf("string", "number")
  makeStyle(CONSTANT) applyTo arrayOf("constant.builtin", "constant")
  makeStyle(ANNOTATION) applyTo arrayOf("attribute")
  makeStyle(FIELD) applyTo arrayOf("variable.field", "variable")
  makeStyle(TYPE_NAME) applyTo arrayOf("type")
